...
static const char *unescape_into(const char *src, char *dst, int maxlen) {
  int si = 0;
...
int toml_parse(const char *path, TomlDoc *doc) {
  ...
  while (fgets(line, sizeof(line), f)) {
    ...
    int klen = (int)(eq - p);
    while (klen > 0 && isspace((unsigned char)p[klen - 1]))
      klen--;
    if (klen <= 0 || klen >= TOML_MAX_STR)
      continue;
    ...
    if (*v == '"') {
      ...
      if (ep != v &&
          (*ep == '\0' || *ep == '#' || isspace((unsigned char)*ep))) {
        ent->val.type = TOML_INT;
        ent->val.i = iv;
      } else {
...
const TomlValue *toml_get(const TomlDoc *doc, const char *section,
  ...
  for (i = 0; i < doc->n; i++) {
    ...
    if (e->table_idx < 0 && strcmp(e->section, section) == 0 &&
        strcmp(e->key, key) == 0)
      return &e->val;
  }
  return NULL;
...
const TomlValue *toml_table_get(const TomlDoc *doc, const char *section,
  ...
  for (i = 0; i < doc->n; i++) {
    const TomlEntry *e = &doc->entries[i];
    if (e->table_idx == idx && strcmp(e->section, section) == 0 &&
        strcmp(e->key, key) == 0)
      return &e->val;
  }
  return NULL;
