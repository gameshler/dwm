history is remembered through a stack list *on each
    monitor.Each client contains a bit array to indicate the tags of a **client
        .****Keys and tagging rules are organized as arrays
            and defined in config.h.****To understand everything else,
    start reading main().** /
...
#include <unistd.h>
#ifdef XINERAMA
#include <X11/ex/* See LICENSE file for copyright and license details.
 *
 ...
 * list on each monitor, the tensions/Xinerama.h>
#endif /* XINERAMA */
...
static const char dwmdir[] = "github/my-dwm/scripts";
...
void attachbottom(Client *c) {
  ...
  // Iterate through the clients to find the last window that is not
  // "chatterino"
...
void sendmon(Client *c, Monitor *m) {
  ...
  if (!(c->tags & montags)) {
    /* If client's current tags don't work on target monitor, assign first valid
     * tag */
    for (i = 0; i < LENGTH(tags); i++) {
      if (montags & (1 << i)) {
        c->tags = 1 << i;
...
void fullscreen(const Arg *arg) {
  ...
  if (selmon->showbar || last_layout == NULL) {
    for (monocle_pos = 0, last_layout = (Layout *)layouts;
         !last_layout->arrange || last_layout->arrange != &monocle;
...
void togglefloating(const Arg *arg) {
  ...
  if (selmon->sel->isfullscreen &&
      selmon->sel->fakefullscreen != 1) /* no support for fullscreen windows */
    return;
...
void unmanage(Client *c, int destroyed) {
  ...
  if (!s) {
    ...
    for (top = m->clients; top && (!ISVISIBLE(top) || top->isfloating);
         top = top->next)
      ;
...
int getmonlogicalindex(Monitor *target) {
  Monitor *m;
...
unsigned int getmontagmask(int monnum) {
...
int getmonitorforselectedtag(void) {
