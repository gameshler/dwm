...
# ── Locate theme.toml ──
THEMES_FILE="${XDG_CONFIG_HOME:-$HOME/.config}/dwm/theme.toml"
if [[ ! -f "$THEMES_FILE" ]]; then
    echo "theme-apply: theme.toml not found at $THEMES_FILE" >&2
    exit 1
fi
...
# Ensure custom themes shipped with dwm are present
GHOSTTY_THEMES_SRC="$HOME/.local/share/dwm/ghostty/themes"
...
THEME_ENV_FILE="${XDG_CONFIG_HOME:-$HOME/.config}/dwm/theme-env.sh"
